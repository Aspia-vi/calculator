...
const TARIFFS = Object.freeze({
  dozvonPct: 0.85,
  markirovkaPct: 0.80, // доля базы, для которой применяется маркировка (МАВ + Этикетка)
  tarifMab: 0.60,
...
/* ===== ГЛАВНЫЙ РАСЧЁТ =====
   Логика телефонии:
   - МАВ + Этикетка списывается только за долю звонков, которая реально маркируется
     (80% базы). Поэтому кол-во звонков умножается на 0.80, а не берётся полностью.
   - Тарификация (поминутная оплата) считается от минут РЕАЛЬНО состоявшихся
     разговоров (минуты = дозвоны (85%) * длина разговора).
   Формула: Телефония = Звонки * 0.80 * МАВ_Этикетка + Минуты * Тарификация
*/
function calc() {
  ...
  const stoimostTelefonii = kolZvonkov * TARIFFS.markirovkaPct * TARIFFS.tarifMab + minuty * TARIFFS.tarifTarifikacia;
