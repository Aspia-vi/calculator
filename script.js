...
/* ===== ГЛАВНЫЙ РАСЧЁТ =====
   Логика телефонии:
   - МАВ + Этикетка списывается за КАЖДУЮ совершённую попытку звонка (kolZvonkov),
     независимо от того, был дозвон или нет (100% от общего числа звонков).
   - Тарификация (поминутная оплата) списывается только за минуты РЕАЛЬНО
     состоявшихся разговоров, т.е. рассчитывается от кол-ва дозвонов (85%) * длина.
   - Стоимость Арины также считается от дозвонов (85%), так как тарифицируется
     только за состоявшийся разговор.
*/
function calc() {
  ...
  const stoimostTelefonii = kolZvonkov * TARIFFS.tarifMab + minuty * TARIFFS.tarifTarifikacia;
